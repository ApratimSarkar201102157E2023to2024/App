     ...
     function removeGame(){
      ...
      game
       ...
       return batch.commit();
        ...
        function handlePayment(token, amount, recipient) {
          // Initialize Stripe
          
        
          // Create a Payment Intent on the server and obtain a client secret
          return fetch('http://localhost:5506', 
          {
            method: 'POST',
            headers: {
          ...
          .then(function(data) {
            // Confirm the Payment Intent using the client secret and token
